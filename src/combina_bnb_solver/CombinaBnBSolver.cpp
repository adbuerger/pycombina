    ...
    while(!node_queue->empty() && !termination_criterion_reached(n_iter, t_start)) {
        ...
        if(active_node->get_lb() < ub_bnb) {
            ...
            if(active_node->get_depth() == n_t) {
                ...
                if(monitor_) {
                    monitor_->on_change(active_node, NodeState::NODE_INTEGER);
                }
            }
            else {
            ...
            if(monitor_) {
                monitor_->on_change(active_node, NodeState::NODE_FATHOMED);
            }
        }
...
void CombinaBnBSolver::add_nodes_to_queue(const NodePtr& parent_node) {
    ...
    if(parent_node && monitor_) {
        if(!node_feasible) {
            monitor_->on_change(parent_node, NodeState::NODE_INFEASIBLE);
        }
        else if(children.empty()) {
            monitor_->on_change(parent_node, NodeState::NODE_FATHOMED);
        }
        else {
            monitor_->on_change(parent_node, NodeState::NODE_SOLVED);
        }
    }
}
