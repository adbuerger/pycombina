...
void bnb_jung(bnb_input *bnb_data_init, bnb_output *bnb_output_data){
    ...
    // Paralellization candidate? Might pay off for bigger problems-->

    double delta_b_opt_p_k_true [n_b];
    double delta_b_opt_p_k_false [n_b];

    delta_b_opt_p_k_true[n_b-1] = b_init[n_b-1] - 1;
    delta_b_opt_p_k_false[n_b-1] = b_init[n_b-1];

    for (i = n_b-2; i >=0; i--){
        delta_b_opt_p_k_true[i] = delta_b_opt_p_k_false[i+1] + b_init[i] - 1;
        delta_b_opt_p_k_false[i] = delta_b_opt_p_k_false[i+1] + b_init[i];
    }
    ...
    while(!q.empty()){
        ...
        if (a.d == n_b){
            ...
            cout << setprecision(15) << "Duration BnB phase: " << double(t_end - t_start) / CLOCKS_PER_SEC << " s\n";
            cout << "eta = " << a.eta << "\n";
            ...
            bnb_output_data->b_data = (int *) malloc(n_b * sizeof(int));
            bnb_output_data->b_data = (int *) memcpy(bnb_output_data->b_data, b_opt, n_b * sizeof(int));

            t_end = clock();

            cout << setprecision(15) << "Duration reconstruction phase: " << double(t_end - t_start) / CLOCKS_PER_SEC << " s\n";
